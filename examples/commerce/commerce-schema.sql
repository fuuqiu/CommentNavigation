-- # 电商平台 · 76 张表的结构导航
-- 来源：macrozheng/mall，Apache-2.0；详见 README.md 与 licenses/mall-Apache-2.0.txt。
-- 基线：dcaa93b3150352e5044708d7211b5bed0af4509f。
-- 修改：仅提取建表语句，增加分层标题，重置自增起点；不包含上游测试数据。
-- 演示用途：MySQL 8；如需执行，请使用独立的空数据库。
...
-- ### 存储产品参数信息的表 · pms_product_attribute_value
...
-- ### 产品的分类和属性的关系表，用于设置分类筛选条件（只支持一级分类） · pms_product_category_attribute_relation
...
-- ### 产品满减表(只针对同商品) · pms_product_full_reduction
...
-- ### 产品阶梯价格表(只针对同商品) · pms_product_ladder
...
-- ### pms_product_operate_log · pms_product_operate_log
...
-- ### 商品审核记录 · pms_product_vertify_record
...
-- ### 订单操作历史记录 · oms_order_operate_history
...
-- ### 优惠券和产品分类关系表 · sms_coupon_product_category_relation
...
-- ### 优惠券和产品的关系表 · sms_coupon_product_relation
...
-- ### 限时购通知记录 · sms_flash_promotion_log
...
-- ### 商品限时购与商品关系表 · sms_flash_promotion_product_relation
...
-- ### 限时购场次表 · sms_flash_promotion_session
...
-- ### 首页推荐专题表 · sms_home_recommend_subject
...
-- ### 后台用户和权限关系表(除角色中定义的权限以外的加减权限) · ums_admin_permission_relation
...
-- ### 后台用户和角色关系表 · ums_admin_role_relation
...
-- ### 成长值变化历史记录表 · ums_growth_change_history
...
-- ### 积分消费设置 · ums_integration_consume_setting
...
-- ### 用户和标签关系表 · ums_member_member_tag_relation
...
-- ### 会员与产品分类关系表（用户喜欢的分类） · ums_member_product_category_relation
...
-- ### 会员统计信息 · ums_member_statistics_info
...
-- ### 后台角色菜单关系表 · ums_role_menu_relation
...
-- ### 后台用户角色和权限关系表 · ums_role_permission_relation
...
-- ### 后台角色资源关系表 · ums_role_resource_relation
...
-- ### 优选专区和产品关系表 · cms_prefrence_area_product_relation
...
-- ### 专题商品关系表 · cms_subject_product_relation
